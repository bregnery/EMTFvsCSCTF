...
Plots::Plots(Sample* insample, Cuts* cut, SampleHistos* inhistos, TString cutName)
{
  ...
  //   iPos = 10*(alignement 1/2/3) + position (1/2/3 = left/center/right)
   ...
   for(std::vector<std::vector<TH1F*> >::const_iterator itr = histos->histo1D.begin(); itr != histos->histo1D.end(); itr++){
  	...
  	createCanvas( iPeriod, 0, *j);   // out of frame (in exceptional cases)
...
void Plots::createCanvas( int iPeriod, int iPos , TH1F* h)
{ 
...
*/
  ...
  TString histoVarName(h->GetName());

  canv->SaveAs("Hist_" + histoVarName + ".png");
